...
void parse_UndirectedEdgeListDarpaHive(graph * G, char *fileName) {
    ...
    printf("WARNING: Weights will be converted to positive real numbers\n");
    int nthreads = 0;
...
#pragma omp parallel
    ...
    while(!feof(file))
    {
        fscanf(file, "%ld %ld %lf", &nv1, &nv2, &wt);
        //printf("%ld %ld %lf\n", nv1, nv2, wt);
        if(nv1 == 0 || nv2 == 0)
            isZeroVertex = 1;
        if(nv1 > NV)
